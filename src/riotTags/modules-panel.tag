    ...
    .flexrow.tall
        .c3.borderright.tall
            ...
            ul#modulelist
                li(each='{module in allModules}', onclick='{renderModule(module)}')
                    svg.feather.in(class='{(module global.currentProject.libs)? \'success on\' : \'off\'}')
                        use(
                            xlink:href='data/icons.svg#{(module in global.currentProject.libs)? \'check\' : \'ctmod\'}'
                        )
                    span {module}
            label.file.flexfix-footer.nmb
                input(type='file', ref='importmodules', accept='.zip', onchange='{importModules}')
        ...
        .c9.tall.flexfix(if='{currentModule}')
            ul.nav.tabs.flexfix-header.noshrink
                li#modinfo.tab(onclick='{changeTab(\'moduleinfo\')}', class='{active: === \'moduleinfo\'}')
                    svg.feather
                        use(xlink:href='data/icons.svg#info')
                    span {voc.info}
                li#modsettings.tab(
                    if='{currentModule.fields && currentModuleName in global.currentProject.libs}',
                    onclick='{changeTab(\'modulesettings\')}',
                    class='{active: === \'modulesettings\'}'
                )
                    svg.feather
                        use(xlink:href='data/icons.svg#settings')
                    span {voc.settings}
                li#modhelp.tab(
                    if='{currentModuleDocs}',
                    onclick='{changeTab(\'modulehelp\')}',
                    class='{active: === \'modulehelp\'}'
                )
                    svg.feather
                        use(xlink:href='data/icons.svg#file-text')
                    span {voc.help}
                li#modlogs.tab(
                    if='{currentModuleLogs}',
                    onclick='{changeTab(\'modulelogs\')}',
                    class='{active: === \'modulelogs\'}'
                )
                    svg.feather
            ...
            .flexfix-body
                #moduleinfo.tabbed.nbt(show='{tab === \'moduleinfo\'}')
                    label.bigpower.in(
                        onclick='{toggleModule(currentModuleName)}',
                        class='{off: !(currentModuleName global.currentProject.libs)}'
                    )
                        svg.feather
                            use(
                                xlink:href='data/icons.svg#{currentModuleName in global.currentProject.libs? \'check\' : \'x\'}'
                            )
                        span
                    h1#modname
                    ...
                    a.external(
                        each='{author in currentModule.main.authors}',
                        title='{voc.author}',
                    ...
                    span(title='{voc.hasinjects}', show='{currentModule.injects}')
                        svg#modinjects.feather.warning
                    ...
                    div(if='{currentModule.dependencies && currentModule.dependencies.length}')
                        ...
                        .inlineblock(each='{dependency in currentModule.dependencies}')
                            svg.feather.success(if='{dependency in global.currentProject.libs}')
                                use(xlink:href='data/icons.svg#check')
                            svg.feather.error(if='{!(dependency in global.currentProject.libs)}')
                    ...
                    div(if='{currentModule.optionalDependencies && currentModule.optionalDependencies.length}')
                        ...
                        .inlineblock(each='{dependency in currentModule.optionalDependencies}')
                            svg.feather.success(if='{dependency in global.currentProject.libs}')
                                use(xlink:href='data/icons.svg#check')
                            svg.feather.warning(if='{!(dependency in global.currentProject.libs)}')
                ...
                #modulesettings.tabbed.nbt(
                    show='{tab === \'modulesettings\'}',
                    if='{currentModule.fields && currentModuleName in global.currentProject.libs}'
                )
