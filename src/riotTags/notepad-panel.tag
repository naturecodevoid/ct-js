notepad-panel#notepad.panel.dockright(class='{opened: opened}')
    ul.nav.tabs.nogrow.nb
        li.tab(onclick='{changeTab(\'notepadlocal\')}', class='{active: === \'notepadlocal\'}')
            svg.feather
                use(xlink:href='data/icons.svg#edit')
            span {voc.local}
        li.tab(onclick='{changeTab(\'notepadglobal\')}', class='{active: === \'notepadglobal\'}')
            svg.feather
                use(xlink:href='data/icons.svg#clipboard')
            span {voc.global}
        li.tab(onclick='{changeTab(\'helppages\')}', class='{active: === \'helppages\'}')
            svg.feather
    ...
    div
        ...
        div(show='{tab === \'helppages\'}')
            iframe(
                src='http://localhost:{server.address().port}/{getIfDarkTheme()? \'?darkTheme=yep\' : \'\'}',
                ref='helpIframe',
                nwdisable,
                nwfaketop
            )
