...
context-menu(class='{opened: opts.menu.opened}', ref='root')
    a(
        ...
        svg.context-menu-anIcon(
            if='{item.icon && item.type !== \'separator\' && item.type !== \'checkbox\'}',
            class='{item.iconClass || \'feather\'}'
        )
            use(xlink:href='data/icons.svg#{item.icon instanceof Function? item.icon() : item.icon}')
        input(
            type='checkbox',
            checked='{item.checked instanceof Function? item.checked() : item.checked}',
