font-editor.panel.view
    .panel.pad.left.tall.flexfix
        .flexfix-body
            label.block
                ...
                input.wide(
                    type='text',
                    onchange='{wire(\'this.fontobj.typefaceName\')}',
                    value='{fontobj.typefaceName}'
                )
            label.block
                b {voc.fontweight}
                br
    ...
    .right.tall(style='font-weight: {fontobj.weight}; font-style: {fontobj.italic? \'italic\' : \'normal\'}')
        each val in [8, 9, 10, 11, 12, 14, 16, 21, 24, 32, 48, 60, 72]
            p(
                style=`font-size: ${val}px; line-height: ${val}px; font-family: 'CTPROJFONT{fontobj.typefaceName}';`,
                data-size=val
