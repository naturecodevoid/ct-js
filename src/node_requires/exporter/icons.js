...
const resizeTo = async function (img, length, dest, soft) {
    ...
    ctx.drawImage(img, (length - img.width * k) / 2, (length - img.height * k) / 2, img.width * k, img.height * k);
...
const bakeFavicons = async function (proj, writeDir) {
    ...
    const img = await getDOMImage(proj.settings.branding.icon, 'ct_ide.png'),
        fsPath = proj.settings.branding.icon
            ? getTextureOrig(proj.settings.branding.icon, true)
            : path.resolve('ct_ide.png');
    const promises = [];
    ...
    promises.push(
        fs
            .readFile(fsPath)
            .then(buff =>
                png2icons.createICO(buff, proj.settings.pixelatedrender ? png2icons.BILINEAR : png2icons.HERMITE)
            )
