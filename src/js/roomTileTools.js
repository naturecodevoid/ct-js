    ...
    window.roomTileTools = {
        init() {
            this.onCanvasMouseUpTiles = e => {
                if (
                    ...
                    for (const tile of this.currentTileLayer.tiles) {
                        ...
                        if (tile.x > xmin && tile.x + g.width < xmax && tile.y > ymin && tile.y + g.height < ymax) {
            ...
            this.onCanvasClickTiles = e => {
                if (Math.hypot(e.offsetX - this.startx, e.offsetY - this.starty) > clickThreshold && !e.shiftKey) {
                    return; // this looks neither like a regular click nor like a Shift+drag
                }

                // cancel potential tile selection on click
                if (this.selectedTiles && !this.movingStuff && !(e.shiftKey && !this.currentTileset)) {
                        ...
                        `
                                ...
                                .then(e => {
                                    if (e.buttonClicked === 'ok') {
                                        ...
                                        for (const tile of this.selectedTiles) {
                                            this.currentTileLayer.tiles.splice(
                                                this.currentTileLayer.tiles.indexOf(tile),
                                                1
                                            );
